 ...
 */
#ifndef KALYPSSO_CORE_MODELS_EULER_EQUATIONS_H_
#define KALYPSSO_CORE_MODELS_EULER_EQUATIONS_H_
...
struct EulerEquations<2>
{
  ...
  compute_pressure(const State & q, real_t gamma0)
  {
   ...
   */
  static KOKKOS_INLINE_FUNCTION real_t
  compute_speed_of_sound(const State & w, real_t gamma0)
  {
  ...
  convert_to_primitive(const State & q, State & w, real_t gamma0)
  {
   ...
   */
  static KOKKOS_INLINE_FUNCTION void
  flux_x(const State & q, real_t p, State & flux)
  {
    flux[ID] = q[IU];                       // rho u
   ...
   */
  static KOKKOS_INLINE_FUNCTION void
  flux_y(const State & q, real_t p, State & flux)
  {
    flux[ID] = q[IV];                       // rho v
   ...
   */
  static KOKKOS_INLINE_FUNCTION void
  flux_visc_x(const GradTensor & g, const Vector & v, const Vector & f, real_t mu, State & flux)
  {
    real_t tau_xx = 2 * mu * (g[U_X] - HALF_F * (g[U_X] + g[V_Y]));
   ...
   */
  static KOKKOS_INLINE_FUNCTION void
  flux_visc_y(const GradTensor & g, const Vector & v, const Vector & f, real_t mu, State & flux)
  {
    real_t tau_yy = 2 * mu * (g[V_Y] - HALF_F * (g[U_X] + g[V_Y]));
    real_t tau_xy = mu * (g[V_X] + g[U_Y]);
  ...
  template <int dir>
  static KOKKOS_INLINE_FUNCTION void
  cons_to_charac(State & data, const State & q, const real_t c, const real_t gamma0)
  {
    State tmp;
  ...
  template <int dir>
  static KOKKOS_INLINE_FUNCTION void
  charac_to_cons(State & data, const State & q, const real_t c, const real_t gamma0)
  {
...
struct EulerEquations<3>
{
  ...
  static constexpr int nbvar = 2 + 3;

  //! type alias to a small array holding hydrodynamics state variables
  using State = HydroState<3, nbvar>;
   ...
   */
  static KOKKOS_INLINE_FUNCTION void
  flux_z(const State & q, real_t p, State & flux)
  {
    real_t w = q[IW] / q[ID];
  ...
  flux_visc_x(const GradTensor & g, const Vector & v, const Vector & f, real_t mu, State & flux)
  {
    real_t tau_xx = 2 * mu * (g[U_X] - ONE_THIRD_F * (g[U_X] + g[V_Y] + g[W_Z]));
  ...
  flux_visc_y(const GradTensor & g, const Vector & v, const Vector & f, real_t mu, State & flux)
  {
    ...
    real_t tau_yy = 2 * mu * (g[V_Y] - ONE_THIRD_F * (g[U_X] + g[V_Y] + g[W_Z]));
   ...
   */
  static KOKKOS_INLINE_FUNCTION void
  flux_visc_z(const GradTensor & g, const Vector & v, const Vector & f, real_t mu, State & flux)
  {
    real_t tau_xz = mu * (g[U_Z] + g[W_X]);
    real_t tau_yz = mu * (g[V_Z] + g[W_Y]);
    real_t tau_zz = 2 * mu * (g[W_Z] - ONE_THIRD_F * (g[U_X] + g[V_Y] + g[W_Z]));
...
#endif // KALYPSSO_CORE_MODELS_EULER_EQUATIONS_H_
