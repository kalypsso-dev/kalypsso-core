...
SolverBase::should_save_solution()
{
  ...
  real_t interval = (m_tEnd - m_tBegin) / static_cast<real_t>(m_output_params.nOutput);

  // m_output_params.nOutput == 0 means no output at all
  if (m_output_params.nOutput == 0)
  {
    return false;
  }

  // m_output_params.nOutput < 0  means always output
  if (m_output_params.nOutput < 0)
  {
    return true;
  }
  ...
  else if (m_output_params.output_policy == +OUTPUT_POLICY::EQUISPACED and
           (m_t - m_tBegin) > (static_cast<real_t>(m_times_saved + 1) * interval))
  {
    return true;
  }
  else if (m_output_params.output_policy == +OUTPUT_POLICY::PREDEFINED)
  {
    const auto times_saved = static_cast<size_t>(m_times_saved);
    if (times_saved < m_output_params.output_times.size())
    {
      if ((m_t - m_tBegin) > m_output_params.output_times[times_saved])
      {
        return true;
      }
    }
