...
MieGruneisenEosArray<device_t>::material_gruneisen_param(size_t i_mat, real_t rho) const
{
  // get EOS type
...
template <typename device_t>
KOKKOS_FUNCTION real_t
MieGruneisenEosArray<device_t>::material_specific_eint_ref(size_t i_mat, real_t rho) const
{
  // get EOS type
  const auto eos_type = MG_EOS_TYPE::_from_integral_unchecked(m_material_eos_type(i_mat));
...
template <typename device_t>
KOKKOS_FUNCTION real_t
MieGruneisenEosArray<device_t>::material_pressure_ref(size_t i_mat, real_t rho) const
{
  // get EOS type
  const auto eos_type = MG_EOS_TYPE::_from_integral_unchecked(m_material_eos_type(i_mat));
...
template <typename device_t>
KOKKOS_FUNCTION real_t
MieGruneisenEosArray<device_t>::material_sound_speed_square(size_t i_mat,
                                                            real_t pressure,
                                                            real_t rho) const
{
  // get EOS type
...
template <typename device_t>
KOKKOS_FUNCTION real_t
MieGruneisenEosArray<device_t>::material_bulk_modulus(size_t i_mat,
                                                      real_t pressure,
                                                      real_t rho) const
{
...
template <typename device_t>
KOKKOS_FUNCTION real_t
MieGruneisenEosArray<device_t>::material_specific_eint_from_pressure(size_t i_mat,
                                                                     real_t pressure,
                                                                     real_t rho) const
{
...
template <typename device_t>
KOKKOS_FUNCTION real_t
MieGruneisenEosArray<device_t>::material_pressure_from_specific_eint(size_t i_mat,
                                                                     ...
                                                                     real_t rho) const
{
  ...
  if (eos_type == +MG_EOS_TYPE::MG_IDEAL_GAS)
  {
    return m_mg_eos_ig(material_eos_id).pressure_from_specific_eint(eint_specific, rho);
  }
  else if (eos_type == +MG_EOS_TYPE::MG_STIFFENED_GAS)
  {
    return m_mg_eos_sg(material_eos_id).pressure_from_specific_eint(eint_specific, rho);
  }
  else if (eos_type == +MG_EOS_TYPE::MG_VANDERWAALS_GAS)
  {
    return m_mg_eos_vdw(material_eos_id).pressure_from_specific_eint(eint_specific, rho);
  }
  else if (eos_type == +MG_EOS_TYPE::MG_SHOCKWAVE)
  {
    return m_mg_eos_sw(material_eos_id).pressure_from_specific_eint(eint_specific, rho);
  }
  else if (eos_type == +MG_EOS_TYPE::MG_SHOCKWAVE2)
  {
    return m_mg_eos_sw2(material_eos_id).pressure_from_specific_eint(eint_specific, rho);
  }
  else if (eos_type == +MG_EOS_TYPE::MG_JWL)
  {
    return m_mg_eos_jwl(material_eos_id).pressure_from_specific_eint(eint_specific, rho);
  }
  else if (eos_type == +MG_EOS_TYPE::MG_COCHRAN_CHAN)
  {
    return m_mg_eos_cc(material_eos_id).pressure_from_specific_eint(eint_specific, rho);
  }

  return ZERO_F;

} // MieGruneisenEosArray<device_t>::material_pressure_from_specific_eint
