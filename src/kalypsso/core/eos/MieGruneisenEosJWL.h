 ...
 *
 */
#ifndef KALYPSSO_CORE_EOS_MIE_GRUNEISEN_EOS_JWL_H_
#define KALYPSSO_CORE_EOS_MIE_GRUNEISEN_EOS_JWL_H_
 ...
 *
 * Define all configurable parameters used in Mie-Gruneisen for of JWL (Jones-Wilkins-Lee) EOS.
 */
struct MieGruneisenEosJWLParam
{
  ...
  {
    const auto material_id = "material" + std::to_string(i_mat) + "_mie_gruneisen_jwl";

    MieGruneisenEosJWLParam params;
    ...
    params.R1 = config_map.getReal(material_id, "R1", KALYPSSO_NUM(1.0));
    params.R2 = config_map.getReal(material_id, "R2", KALYPSSO_NUM(1.0));
...
// ==============================================================================
/**
 * JWL (Jones-Wilkins-Lee) EOS written in Mie-Gruneisen for.
 *
...
struct MieGruneisenEosJWL
{
   ...
   */
  MieGruneisenEosJWL(const size_t i_mat, const ConfigMap & config_map)
    : m_params(MieGruneisenEosJWLParam::get_parameters(i_mat, config_map))
  {}
  ...
  pressure_ref(const real_t rho) const
  {
    ...
    return A1 * exp(-R1 * rho0 / rho) + A2 * exp(-R2 * rho0 / rho);
  } // pressure_ref
  ...
  dpressure_ref_drho(const real_t rho) const
  {
    ...
    // clang-format off
    return (A1 * R1 * rho0 * exp(-R1 * rho0 / rho) +
            A2 * R2 * rho0 * exp(-R2 * rho0 / rho)) / rho2;
    // clang-format on
  } // dpressure_ref_drho
  ...
  eint_ref(const real_t rho) const
  {
    ...
    // clang-format off
    return (A1 / R1 * exp(-R1 * rho0 / rho) +
            A2 / R2 * exp(-R2 * rho0 / rho)) / rho0 + e0;
    // clang-format on
  } // eint_ref
  ...
  deint_ref_drho(const real_t rho) const
  {
    ...
    // clang-format off
    return (A1 * exp(-R1 * rho0 / rho) +
            A2 * exp(-R2 * rho0 / rho)) / rho2;
    // clang-format on
...
#endif // KALYPSSO_CORE_EOS_MIE_GRUNEISEN_EOS_JWL_H_
