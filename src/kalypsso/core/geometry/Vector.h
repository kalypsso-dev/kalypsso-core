 ...
 * \file Triangle.h
 */
#ifndef KALYPSSO_CORE_GEOMETRY_VECTOR_H_
#define KALYPSSO_CORE_GEOMETRY_VECTOR_H_
...
struct Vector
{
  ...
  operator==(Vector const & v, Vector const & w)
  {
    bool match = true;
    for (int d = 0; d < DIM; ++d)
      match &= (v[d] == w[d]);
...
normalize(Vector<DIM, Coordinate> const & v)
{
...
template <typename Point,
          ...
          typename Enable = std::enable_if_t<GeometryTraits::is_point_v<Point> &&
                                             GeometryTraits::is_point_v<Point2>>>
KOKKOS_INLINE_FUNCTION constexpr auto
operator-(Point const & end, Point2 const & begin)
{
  namespace GT = GeometryTraits;
  constexpr int DIM = GT::dimension_v<Point>;
  static_assert(GT::dimension_v<Point2> == DIM);
  geometry::Vector<DIM, GT::coordinate_type_t<Point>> v;
  for (int d = 0; d < DIM; ++d)
    v[d] = end[d] - begin[d];
  return v;
}
...
#endif // KALYPSSO_CORE_GEOMETRY_VECTOR_H_
