...
/**
 * \file morton_utils.h
 * Some useful routines to compute / handle Morton indexes.
 *
 ...
 *
 */
#ifndef KALYPSSO_CORE_MORTON_UTILS_H
#define KALYPSSO_CORE_MORTON_UTILS_H

#include <cstdint> // for uint32_t, uint64_t, etc...
#include <cstddef> // for std::size_t

#include <kalypsso/core/enums.h> // for ComponentIndex3D
 ...
 */
template <size_t dim>
KOKKOS_INLINE_FUNCTION uint64_t
splitBy3(uint32_t a);

/* 1D version */
template <>
KOKKOS_INLINE_FUNCTION uint64_t
splitBy3<1>(uint32_t a)
{
...
template <>
KOKKOS_INLINE_FUNCTION uint64_t
splitBy3<2>(uint32_t a)
{

  // we take all 32 bits (this is different in 3D)
  // because the result must hold in an uint64_t ( 2x21=42 bits --> uint64_t is enough )
...
template <size_t dim>
KOKKOS_INLINE_FUNCTION uint64_t
compute_morton_key(const index_t<dim> & index)
{
  uint64_t key = 0;
  if (dim == 1)
  {
...
KOKKOS_INLINE_FUNCTION
uint64_t
compute_morton_key(const uint32_t ix, const uint32_t iy)
{
  uint64_t key = 0;
...
KOKKOS_INLINE_FUNCTION
uint64_t
compute_morton_key(const uint32_t ix, const uint32_t iy, const uint32_t iz)
{
  uint64_t key = 0;
 ...
 */
template <size_t dim, int coord>
KOKKOS_INLINE_FUNCTION uint32_t
morton_extract_bits(uint64_t key)
{
...
KOKKOS_INLINE_FUNCTION
uint64_t
get_neighbor_morton(uint64_t key, uint8_t level, uint8_t face)
{
...
KOKKOS_INLINE_FUNCTION
uint64_t
get_neighbor_morton(uint64_t key, uint8_t level, uint8_t level_n, uint8_t face, uint8_t neigh_id)
{
  ...
  if (level_n != level)
  {
    ...
    if (dir == IX)
    {
      ...
      if (iface == 0)
      {
        xyz[IX] = (xyz[IX] < length_n) ? xyz[IX] + total_length - length_n : xyz[IX] - length_n;
      }
      else
      {
        xyz[IX] += length;
    ...
    if (dir == IY)
    {
      if (iface == 0)
      {
        xyz[IY] = (xyz[IY] < length_n) ? xyz[IY] + total_length - length_n : xyz[IY] - length_n;
      }
      else
      {
        xyz[IY] += length;
    ...
    if (dir == IZ)
    {
      if (iface == 0)
      {
        xyz[IZ] = (xyz[IZ] < length_n) ? xyz[IZ] + total_length - length_n : xyz[IZ] - length_n;
      }
      else
      {
        xyz[IZ] += length;
...
#endif // KALYPSSO_CORE_MORTON_UTILS_H
