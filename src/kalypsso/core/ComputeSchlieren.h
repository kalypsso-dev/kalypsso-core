...
/**
 * Define parameters that are only useful when SCHLIEREN_SCALING is EXP.
 *
 * reference:
 * Role of Atwood number on flow morphology of a planar shock-accelerated square bubble:A
 * numerical study, Satyvir Singha, Phys. Fluids 32, 126112 (2020); doi: 10.1063/5.0031698
 * See section III,  equation (6)
 */
struct SchlierenParams
{
  //! index to input array where to find density
  int ivar_rho;

  //! index to input array where to find volume fraction
  int ivar_vol_frac;

  //! volume fraction threshold
  real_t vol_frac_th;

  //! scaling factor for low volume fraction
  real_t scale_low;

  //! scaling factor for high volume fraction
  real_t scale_high;

  SchlierenParams(ConfigMap const & config_map)
    : ivar_rho(config_map.getInteger("schlieren", "ivar_density", 0))
    , ivar_vol_frac(config_map.getInteger("schlieren", "ivar_vol_frac", 0))
    , vol_frac_th(config_map.getReal("schlieren", "vol_frac_th", KALYPSSO_NUM(0.25)))
    , scale_low(config_map.getReal("schlieren", "scale_low", KALYPSSO_NUM(100.0)))
    , scale_high(config_map.getReal("schlieren", "scale_high", KALYPSSO_NUM(20.0)))
  {}
}; // struct SchlierenParams
...
public:
  ...
  //!
  ComputeSchlieren(ConfigMap const &        config_map,
...
private:
  //! Schlieren params
  SchlierenParams m_schlieren_params;
