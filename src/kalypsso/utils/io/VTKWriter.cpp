...
#include <kalypsso/utils/io/libb64/libb64.h> // for type base64 encoding
#include <algorithm>                         // for std::max
#include <ios>                               // for std::ios::good
#include <iostream>                          // for std::cerr

#include <kalypsso/utils/io/VTKWriter.h>
...
// =======================================================
// =======================================================
VTKWriter::VTKWriter(ConfigMap & config_map, int64_t nbCells)
  : m_fileHandler()
  ...
  , m_out_file()
{
  ...
  // set write type
  bool outputVtkAscii = m_config_map.getBool("output", "outputVtkAscii", false);
  bool outputVtkAppended = m_config_map.getBool("output", "outputVtkAppended", false);
  bool outputVtkBinary = m_config_map.getBool("output", "outputVtkBinary", true);
...
VTKWriter::write_header()
{
  ...
  if (m_out_file.is_open())
    write_vtu_header(m_out_file, m_config_map);
  else
...
// =======================================================
void
VTKWriter::write_metadata(int iStep, real_t time)
{
...
// =======================================================
void
VTKWriter::write_cell_data(const std::string & dataname, const std::vector<real_t> & cell_data)
{
  ...
  m_offsetBytes += sizeof(uint64_t) + sizeof(real_t) * static_cast<size_t>(m_nbCells);

  if (vtk_ascii_enabled())
  {

    for (uint64_t i = 0; i < static_cast<uint64_t>(m_nbCells); ++i)
    {
  ...
  if (vtk_binary_enabled())
  {
    ...
    write_base64_binary_data(reinterpret_cast<const char *>(&(cell_data[0])),
                             sizeof(real_t) * static_cast<size_t>(m_nbCells));
...
// ================================================================
void
VTKWriter::write_appended_binary_cell_data([[maybe_unused]] const std::string & dataname,
                                           const std::vector<real_t> &          cell_data)
{
   ...
   */
  {
    ...
    auto size = sizeof(real_t) * static_cast<size_t>(m_nbCells);
    m_out_file.write(reinterpret_cast<char *>(&size), sizeof(uint64_t));
    m_out_file.write(reinterpret_cast<const char *>(&(cell_data[0])), static_cast<int64_t>(size));
...
VTKWriter::write_base64_binary_data(const char * numeric_data, size_t byte_length)
{
  ...
  chunksize = static_cast<size_t>(1) << 15; /* 32768 */
  int_header = static_cast<uint32_t>(byte_length);

  /* Allocate sufficient memory for base64 encoder */
  code_length = 2 * std::max(chunksize, sizeof(int_header));
  code_length = std::max(code_length, static_cast<size_t>(4)) + 1;
  base_data = new char[code_length];

  base64_init_encodestate(&encode_state);
  base_length = base64_encode_block(
    reinterpret_cast<char *>(&int_header), sizeof(int_header), base_data, &encode_state);
  // assert (base_length < code_length);
  base_data[base_length] = '\0';
  m_out_file.write(base_data, static_cast<int64_t>(base_length));
