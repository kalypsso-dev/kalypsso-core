...
#ifndef KALYPSSO_UTILS_IO_VTKWRITER_H
#define KALYPSSO_UTILS_IO_VTKWRITER_H
 ...
 * \class VTKWriter
 *
 * Supporting VTK unstructured grid format.
 *
 * If run in parallel (MPI enabled), each MPI process writes a vtu file,
 * and process 0 (master) writes an additional pvtu (partitioned vtu)
...
protected:
  ...
  ConfigMap &    m_config_map;  /**< application config map */
...
public:
  VTKWriter(ConfigMap & config_map, int64_t nbCells);
  virtual ~VTKWriter();
  ...
  void
  set_vtk_write_type(VTK_WRITE_ENUM wt)
  {
    m_vtk_write_type = wt;
  };
  bool
  ...
  void
  write_metadata(int iStep, real_t time);

  /** Write number of nodes and cells for current piece (MPI sub-domain). */
  void
  write_piece_header(int64_t nbNodes);
  ...
  template <int dim>
  void
  write_geometry(const std::vector<Point<3>> & nodes_location = std::vector<Point<3>>());
  ...
  template <int dim>
  void
  write_appended_binary_geometry(std::vector<Point<3>> & nodes_location);
   ...
   */
  void
  write_appended_binary_cell_data(const std::string &         dataname,
   ...
   */
  int
  write_base64_binary_data(const char * bytes, size_t data_size);
...
template <int dim>
void
VTKWriter::write_geometry(const std::vector<Point<3>> & nodes_location)
{
  ...
  else if (vtk_binary_enabled())
  {
    ...
    for (int64_t i = 0; i < m_nbCells * nbNodesPerCell; ++i)
    {
      ...
      vertices[3 * i] = static_cast<float>(nodes_location[i][IX]);
      vertices[3 * i + 1] = static_cast<float>(nodes_location[i][IY]);
      vertices[3 * i + 2] = static_cast<float>(nodes_location[i][IZ]);
  ...
  m_offsetBytes += sizeof(uint64_t) + sizeof(float) * m_nbCells * nbNodesPerCell * 3;
...
VTKWriter::write_connectivity()
{
   ...
   * CONNECTIVITY
   */
  m_out_file << "    <DataArray type=\"Int64\" Name=\"connectivity\" format=\"" << m_write_type_str
             << "\"";
  ...
  m_offsetBytes += sizeof(uint64_t) + sizeof(uint64_t) * m_nbCells * nbNodesPerCell;
  ...
  if (vtk_binary_enabled())
  {
    ...
    write_base64_binary_data(reinterpret_cast<const char *>(&(offsets_array[0])),
   ...
   * OFFSETS
   */
  m_out_file << "    <DataArray type=\"Int64\" Name=\"offsets\" format=\"" << m_write_type_str
             << "\"";
   ...
   * CELL TYPES
   */
  m_out_file << "    <DataArray type=\"UInt8\" Name=\"types\" format=\"" << m_write_type_str
             << "\"";
  ...
  if (vtk_binary_enabled())
  {
    ...
    for (int64_t i = 0; i < m_nbCells; ++i)
    {
      tmpArray[i] = static_cast<uint8_t>(cellType);
    }
...
template <int dim>
void
VTKWriter::write_appended_binary_geometry(std::vector<Point<3>> & nodes_location)
{
...
#endif // KALYPSSO_UTILS_IO_VTKWRITER_H
