 ...
 *
 */
#ifndef KALYPSSO_UTILS_MPI_MPICOMM_H
#define KALYPSSO_UTILS_MPI_MPICOMM_H

#include <kalypsso/utils/mpi/mpi_utils.h>
#include <mpi.h>
...
public:
  ...
  MPI_Isend(const Kokkos_View_t & view, int dest, int tag) const
  {
    ...
    CHECK_MPI_ERR(::MPI_Isend(
      view.data(), static_cast<int>(view.size()), type, dest, tag, MPI_Comm(*this), &r));
    return r;
  }
  ...
  inline void
  MPI_Waitall(uint32_t count, MPI_Request * requests) const
  {
    CHECK_MPI_ERR(::MPI_Waitall(static_cast<int>(count), requests, MPI_STATUSES_IGNORE));
  }
...
#endif // KALYPSSO_UTILS_MPI_MPICOMM_H
