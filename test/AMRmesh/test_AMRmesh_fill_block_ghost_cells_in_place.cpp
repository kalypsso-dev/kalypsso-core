...
/**
 * \file test_AMRmesh_fill_block_ghost_cells_in_place.cpp
 *
 * Purpose: test and validate code used to fill ghost cells around blocks when input/output data is
 * a DataArrayGhostedBlock object.
 *
 *
 */
...
#include <test_common/FillOutside2.h>
...
run_test(const ParallelEnv & par_env, const ConfigMap & config_map)
{
  ...
  auto userdata_ghosted_block =
    initial_amr_setup.setup_initial_data_ghosted_block(orchard_keys_device, false);
  ...
  //
  {
    std::string filename = dim == 2 ? "test_AMR_fill_block_noghosts_in_place_2d_before"
                                    : "test_AMR_fill_block_noghosts_in_place_3d_before";
  ...
  mesh_ghosts_exchanger.exchange(userdata_ghosted_block.data());
  ...
  //
  {
    ...
    test::FillOutside2CellFunctor<dim, device_t, InitFunc>::apply(config_map,
                                                                  amr_hashmap_device,
                                                                  orchard_keys_device,
                                                                  amr_mesh_info,
  ...
  //
  FillBlockGhostCellsInPlaceFunctor<dim, device_t>::apply(config_map,
                                                          amr_hashmap_device,
                                                          orchard_keys_device,
                                                          userdata_ghosted_block,
                                                          brick_sizes,
  ...
  {
    std::string filename = dim == 2 ? "test_AMR_fill_block_ghosts_in_place_2d_after"
                                    : "test_AMR_fill_block_ghosts_in_place_3d_after";
  ...
  {
    std::string filename = dim == 2 ? "test_AMR_fill_block_ghosts_in_place_2d_diff"
                                    : "test_AMR_fill_block_ghosts_in_place_3d_diff";
...
main(int argc, char ** argv)
{

  {
    ...
    if (kalypsso::cmdline_arg_exists(argv, argv + argc, "--help"))
    {
      ...
      {
        // clang-format off
        std::cout << "Example cmdline: \"mpirun -np 4 ./test_AMRmesh_fill_block_ghost_cells_in_place --ini test_AMRmesh_fill_ghost_brick_2d.ini --refine_level 6\"\n";
        // clang-format on
      }
      return 0;
    }
    ...
    assertm(dim == 2 or dim == 3, "[test_AMRmesh_fill_block_ghost_cells_in_place] Wrong dimension");
