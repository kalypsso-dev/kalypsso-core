...
/**
 * \file test_AMRmesh_fill_block_ghost_faces.cpp
 *
 * Purpose: test and validate code used to fill ghost faces around a block.
 *
...
run_test(const ParallelEnv & par_env, const ConfigMap & config_map)
{
  ...
  auto facedata_block = initial_amr_setup.setup_initial_data_block_face(orchard_keys_device, true);

  // ghostwidth of 2
  const uint32_t ghostwidth = 2;
  const auto     bSize = facedata_block.cell_block_size_inner();
  const auto     num_octs = facedata_block.num_quadrants();
  auto           facedata_block_ghosted =
    FaceDataArrayBlock_t("facedata_block_ghosted", bSize, ghostwidth, num_octs);

  //
  // save face data before filling block ghosts
  //
  {
    std::string filename = dim == 2 ? "test_AMR_fill_block_faces_noghosts_2d_before"
                                    : "test_AMR_fill_block_faces_noghosts_3d_before";

    // provide mapping between variables Id and variables names
    const Hydro_t model(dim);

    DataWriter<dim, device_t, Hydro_t>::save(
      filename, facedata_block, config_map, amr_mesh, "facedata_before_remapping");

    // compute divergence
    const auto divBdata =
      FaceDataArrayBlock_t::compute_divergence(facedata_block, mesh_map->orchard_keys());

    DataWriter<dim, device_t, Hydro_t>::save_scalar(
      filename + std::string("_div"), divBdata, 0, config_map, amr_mesh);
  }
  ...
  mesh_ghosts_exchanger.exchange(facedata_block);
  ...
  //
  // {
  //   using bc_array_t = BorderConditionsConfig<godunov_hydro::BC_HYDRO>::bc_array_t<dim>;
  ...
  //       // clang-format off
  //       return bc_array_t{
  //         godunov_hydro::BC_HYDRO::ANALYTICAL,
  //         godunov_hydro::BC_HYDRO::ANALYTICAL,
  //         godunov_hydro::BC_HYDRO::ANALYTICAL,
  //         godunov_hydro::BC_HYDRO::ANALYTICAL
  //       };
  ...
  //     {
  //       return bc_array_t{ godunov_hydro::BC_HYDRO::ANALYTICAL,
  //       godunov_hydro::BC_HYDRO::ANALYTICAL,
  //                          godunov_hydro::BC_HYDRO::ANALYTICAL,
  //                          godunov_hydro::BC_HYDRO::ANALYTICAL,
  //                          godunov_hydro::BC_HYDRO::ANALYTICAL,
  ...
  //   muscl_mhd::FillOutsideCellFunctor<dim, device_t, InitFunc>::apply(
  //     config_map,
  //     amr_hashmap_device,
  //     orchard_keys_device,
  //     amr_mesh_info,
  ...
  //
  // filling blocks ghost faces (piecewise)
  //
  {
    const auto num_elts = facedata_block_ghosted.num_elements_per_octant();

    const auto nbOcts = amr_mesh_info.local_num_quadrants();

    // process octants in a picewise manner
    const uint32_t num_octants_per_group = 32000;

    const uint32_t nbGroups = (nbOcts + num_octants_per_group - 1) / num_octants_per_group;

    auto facedata_block_ghosted_range =
      FaceDataArrayBlock<dim, real_t, device_t>("facedata_block_ghosted_range",
                                                facedata_block_ghosted.cell_block_size_inner(),
                                                ghostwidth,
                                                num_octants_per_group);

    for (uint32_t iGroup = 0; iGroup < nbGroups; ++iGroup)
    {
      ...
      FillBlockGhostFacesFunctor<dim, device_t>::apply(config_map,
                                                       amr_hashmap_device,
                                                       orchard_keys_device,
                                                       nbOcts,
                                                       iOct_begin,
                                                       ...
                                                       facedata_block,
                                                       facedata_block_ghosted_range,
                                                       brick_sizes,
                                                       is_brick_periodic,
                                                       amr_mesh_info);

      // copy back results into the full array
      {
        // source view
        auto data_range_in =
          std::pair<std::size_t, std::size_t>(0, num_elts * num_octants_to_process);
        auto facedata_block_ghosted_range_view =
          Kokkos::subview(facedata_block_ghosted_range.logical_view(), data_range_in);

        // destination view
        auto data_range_out = std::pair<std::size_t, std::size_t>(
          num_elts * iOct_begin, num_elts * (iOct_begin + num_octants_to_process));
        auto facedata_block_ghosted_subview =
          Kokkos::subview(facedata_block_ghosted.logical_view(), data_range_out);

        // copy
        Kokkos::deep_copy(facedata_block_ghosted_subview, facedata_block_ghosted_range_view);
      }
    }
  }

  {
    std::string filename = dim == 2 ? "test_AMR_fill_block_faces_ghost_2d_after"
                                    : "test_AMR_fill_block_faces_ghost_3d_after";

    // provide mapping between variables Id and variables names
    const Hydro_t model(dim);

    DataWriter<dim, device_t, Hydro_t>::save(
      filename, facedata_block_ghosted, config_map, amr_mesh, "facedata_after_remapping");

    // compute divergence
    const auto divBdata =
      FaceDataArrayBlock_t::compute_divergence(facedata_block_ghosted, mesh_map->orchard_keys());

    DataWriter<dim, device_t, Hydro_t>::save_scalar(
      filename + std::string("_div"), divBdata, 0, config_map, amr_mesh);
  }

  // // create data array with ghost blocks filled with analytical values
  ...
  //   DataWriter<dim, device_t>::template save<Hydro_t>(
  //     filename, diff, config_map, amr_mesh, true, model);
  // }

  // //
  // // perform a reduce to check everything is ok (this is a unit test)
  // //
  // double              l2_square_diff = 0;
  // Kokkos::Sum<double> reducer_diff(l2_square_diff);
  ...
  // double              l2_square_true = 0;
  // Kokkos::Sum<double> reducer_true(l2_square_true);
  ...
  //   Kokkos::MDRangePolicy<exec_space, Kokkos::Rank<3>>(
  //     { 0, 0, 0 },
  //     { (int32_t)userdata_ghosted_block_v.num_cells(),
  //       (int32_t)userdata_ghosted_block_v.num_vars(),
  ...
  // printf("[MPI rank=%d] Relative L2 norm is : %f\n",
  //        par_env.rank(),
...
main(int argc, char ** argv)
{

  {
    ...
    if (kalypsso::cmdline_arg_exists(argv, argv + argc, "--help"))
    {
      ...
      {
        // clang-format off
        std::cout << "Example cmdline: \"mpirun -np 4 ./test_AMRmesh_fill_block_ghost_faces --ini test_AMRmesh_fill_ghost_brick_2d.ini --refine_level 6\"\n";
        // clang-format on
      }
      return 0;
    }
    ...
    assertm(dim == 2 or dim == 3, "[test_AMRmesh_fill_block_ghost_faces] Wrong dimension");
    ...
    using InitFunc = kalypsso::InitFunc3;
