...
/**
 * \file test_AMRmesh_fill_block_ghost_cells_v2.cpp
 *
...
run_test(const ParallelEnv & par_env, const ConfigMap & config_map)
{
  ...
  //
  {
    ...
    for (int32_t iGroup = 0; iGroup < nbGroups; ++iGroup)
    {
      ...
      FillBlockGhostCellsFunctorV2<dim, device_t>::apply(amr_hashmap_device,
                                                         orchard_keys_device,
                                                         nbOcts,
                                                         iOct_begin,
  ...
  Kokkos::parallel_reduce(
    ...
    Kokkos::MDRangePolicy<exec_space, Kokkos::Rank<3>>(
      ...
      { userdata_ghosted_block_v.num_cells(),
        ...
        /*userdata_ghosted_block_v.num_quadrants()*/
...
main(int argc, char ** argv)
{

  {
    ...
    if (kalypsso::cmdline_arg_exists(argv, argv + argc, "--help"))
    {
      ...
      {
        // clang-format off
        std::cout << "Example cmdline: \"mpirun -np 4 ./test_AMRmesh_fill_block_ghost_cells_v2 --ini test_AMRmesh_fill_ghost_brick_2d.ini --refine_level 6\"\n";
        // clang-format on
      }
      return 0;
    }
    ...
    assertm(dim == 2 or dim == 3, "[test_AMRmesh_fill_block_ghost_cells_v2] Wrong dimension");
