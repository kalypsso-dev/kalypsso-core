...
FillOutside2CellFunctor<dim, device_t, AnalyticalBC>::FillOutside2CellFunctor(
...
template <size_t dim, typename device_t, typename AnalyticalBC>
void
FillOutside2CellFunctor<dim, device_t, AnalyticalBC>::apply(
  ConfigMap const &        config_map,
  amr_hashmap_t            amr_hashmap,
  orchard_key_view_t       orchard_keys,
  AMRMeshInfo              amr_mesh_info,
  ...
  bc_array_t               bc_types)
{
  ...
  FillOutside2CellFunctor<dim, device_t> functor(config_map,
                                                 amr_hashmap,
                                                 orchard_keys,
                                                 amr_mesh_info,
                                                 userdata,
  ...
  const auto nbCellsPerLeaf = userdata.num_cells_inner();
  ...
  Kokkos::parallel_for(
    "FillOutside2Functor", Kokkos::RangePolicy<exec_space>(0, totalNumberOfOutsideCells), functor);
...
{
  // create compute functor
  FillOutside2CellFunctor<dim, device_t, AnalyticalBC> functor(config_map,
                                                               amr_hashmap,
                                                               orchard_keys,
                                                               amr_mesh_info,
                                                               userdata,
...
FillOutside2CellFunctor<dim, device_t, AnalyticalBC>::operator()(const index_t & global_index) const
{

  const auto nbCellsPerLeaf = m_userdata.num_cells_inner();
  ...
  KOKKOS_ASSERT(is_valid_key &&
                "[kalypsso::FillOutside2CellFunctor] key_inside does not exist in hashmap !?");
  ...
  KOKKOS_ASSERT(
    ...
    "[kalypsso::FillOutside2CellFunctor] iOct does not identify an octant inside domain ?!");
        ...
        orchard_key_t<dim>::is_outside_dir(key_outside, dir))
    {
      if (m_bc_types[face]._to_integral() == +BC_HYDRO::ZERO_GRADIENT)
      {
        ...
        for (int32_t ivar = 0; ivar < m_userdata.num_vars(); ++ivar)
          m_userdata(coord_out, ivar, iOct_outside) = m_userdata(coord_in, ivar, iOct_inside);
      }
      else if (m_bc_types[face]._to_integral() == +BC_HYDRO::WALL)
      {
        ...
        for (int32_t ivar = 0; ivar < m_userdata.num_vars(); ++ivar)
        {
          ...
          m_userdata(coord_out, ivar, iOct_outside) =
            scale * m_userdata(coord_in, ivar, iOct_inside);
        } // end for ivar
      ...
      else if (m_bc_types[face]._to_integral() == +BC_HYDRO::ANALYTICAL)
      {
        ...
        for (int32_t ivar = 0; ivar < m_userdata.num_vars(); ++ivar)
        {
          if constexpr (dim == 2)
          {
            m_userdata(coord_out, ivar, iOct_outside) = m_f(xyz_cell[IX], xyz_cell[IY], ivar);
          }
...
template class FillOutside2CellFunctor<2, kalypsso::DefaultDevice>;
template class FillOutside2CellFunctor<3, kalypsso::DefaultDevice>;
template class FillOutside2CellFunctor<2, kalypsso::DefaultDevice, InitFunc1>;
template class FillOutside2CellFunctor<3, kalypsso::DefaultDevice, InitFunc1>;
template class FillOutside2CellFunctor<2, kalypsso::DefaultDevice, InitFunc2>;
template class FillOutside2CellFunctor<3, kalypsso::DefaultDevice, InitFunc2>;
