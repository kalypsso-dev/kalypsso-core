 ...
 *
 * The first type should be useful to store all the leaves of tree.
 * The second type could be useful to store additionally level ghost cells.
 *   Indeed this allows to have cells of different sizes covering the same
 *   location (same x,y,z but different levels).
 */
#ifndef KALYPSSO_TEST_SHARED_KEY_TYPE_H
 ...
 * Cell status.
 *
 * - "ghost level cells" are inserted in hashmap, to make all regular cell have
 * a regular neighborhood; they inserted by interpolated the payload data.
 *
 ...
 * hold the very same Morton key, same level, only the payload data is different
 *
 */
enum CellStatus
{
  ...
  CELL_GHOST_LEVEL = 2,    /*!< a cell inserted in hashmap to make neighbor cell surrounded by cells
 ...
 * keys[1] holds additional information:
...
struct amr_key_t
{
  ...
  /** bit mask to extract CELL_STATUS */
  // static constexpr uint64_t CELL_STATUS_MASK = 0x000000000F000000;
  ...
  KOKKOS_INLINE_FUNCTION
  amr_key_t(uint64_t key1, uint64_t key2)
    : keys{ key1, key2 }
  {}
  ...
  KOKKOS_INLINE_FUNCTION
  bool
  operator<(const amr_key_t & other) const
  {
...
KOKKOS_INLINE_FUNCTION
static uint64_t
encode_level_tree(int level, int treeId)
{
  uint64_t res = 0;
...
/**
 * Define the Morton key type encoding the local tree morton index with tree id.
 *
 * keys[0] holds the morton index computed inside the tree the cell belongs to.
 * keys[1] holds additional information:
...
struct morton_key_t
{
  ...
  KOKKOS_INLINE_FUNCTION
  morton_key_t(uint64_t key1, uint64_t key2)
    : keys{ key1, key2 }
  {}
...
#endif // KALYPSSO_TEST_SHARED_KEY_TYPE_H
