...
using DataMap = Kokkos::UnorderedMap<uint64_t, uint64_t, Kokkos::DefaultExecutionSpace>;
...
test_unordered_map_2d(int N)
{
  ...
  for (std::size_t i = 0; i < dataMapOnHost.capacity(); ++i)
  {
    ...
    {
      std::cout << i << " " << "dataMapOnHost[" << dataMapOnHost.key_at(i)
