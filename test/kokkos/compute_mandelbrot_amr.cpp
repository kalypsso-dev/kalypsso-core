...
#include <kalypsso/core/amr_key.h>
...
#include <kalypsso/utils/io/VTKWriter.h> // for VTK_WRITE_ENUM class
#include <kalypsso/utils/io/IO_VTK_shared.h>
...
struct MandelbrotConfig
{
   ...
   */
  using MandelbrotMap = typename Kokkos::UnorderedMap<amr_key_t, metadata_t, device_t>;
};
 ...
 *
 * Not really needed, since the functionality already exists as
 * UnorderedMap method named size().
 *
 */
template <typename device_t>
struct ComputeMapSize : public MandelbrotConfig<device_t>
{

  using exec_space = typename device_t::execution_space;
  using MandelbrotMap_t = typename MandelbrotConfig<device_t>::MandelbrotMap;
  ...
  getSize(MandelbrotMap_t _mandelbrotMap)
  {
    ...
    Kokkos::parallel_reduce("ComputeMapSize",
                            Kokkos::RangePolicy<exec_space>(space, 0, _mandelbrotMap.capacity()),
                            functor,
                            size);
    space.fence();
    return size;
  ...
  void
  join(uint32_t & update, const uint32_t & input) const
  {
    update += input;
  }
#else
 ...
 */
template <typename device_t>
struct FillCoarseMap : public MandelbrotConfig<device_t>
{
  ...
  {
    int size = N * N;
    Kokkos::parallel_for("FillCoarseMap", Kokkos::RangePolicy<exec_space>(0, size), *this);
  }
 ...
 *
 * the refinement criterion is adapted from the Mariani-Silver Algorithm, i.e.
 ...
 */
template <typename device_t>
struct Refine : public MandelbrotConfig<device_t>
{
  ...
  apply(MandelbrotMap_t _mandelbrotMap, int level_min, int level_max)
  {
    ...
    Refine functor(_mandelbrotMap, level_min, level_max, Refine::INSERT_REFINED_CELLS);
    ...
    for (int iter = 0; iter < nbIter; ++iter)
    {
      ...
      // perform refinement
      Kokkos::parallel_for("Refine::INSERT_REFINED_CELLS",
                           Kokkos::RangePolicy<exec_space>(0, _mandelbrotMap.capacity()),
                           functor);
      ...
      if (VERBOSE)
        std::cout << "is data map erasable ? " << _mandelbrotMap.erasable() << "\n";
      ...
      int map_size = ComputeMapSize<device_t>::getSize(_mandelbrotMap);
      printf("[iter %d] Map size=%d (end)\n", iter, map_size);
  ...
  KOKKOS_INLINE_FUNCTION
  void
  delete_old_cells(const int & i) const
  {
  ...
  KOKKOS_INLINE_FUNCTION
  void
  reset_status(const int & i) const
  {
 ...
 */
template <typename device_t>
struct VtkWriter : public MandelbrotConfig<device_t>
{
  ...
  dump(const std::string & filename, MandelbrotMap_t mandelbrotMap)
  {
    ...
    std::vector<kalypsso::Point<3>> nodes_coord(nbNodes);
    ...
    using CellData = typename Kokkos::View<double *, device_t>;
    ...
    Kokkos::parallel_scan(
      ...
      Kokkos::RangePolicy<exec_space>(0, mandelbrotMap.capacity()),
    ...
    Kokkos::parallel_for(
      "get nodes positions",
      Kokkos::RangePolicy<exec_space>(0, keyVec.extent(0)),
    ...
    ConfigMap config_map("./test_io_vtk_2d.ini");
    config_map.setString("output", "outputPrefix", "mandelbrot_set");
    kalypsso::io::VTKWriter vtkWriter(config_map, nbCells);
    vtkWriter.open_file();
 ...
 */
template <typename device_t>
struct MandelbrotCompute : public MandelbrotConfig<device_t>
{
  using MandelbrotMap_t = typename MandelbrotConfig<device_t>::MandelbrotMap;
  using exec_space = typename device_t::execution_space;
  ...
  run(int level_min, int level_max, int max_capacity_prefactor)
  {
    ...
    // maximum capacity of the hash map container
    ...
    FillCoarseMap<device_t> fill(mandelbrotMap, level_min, level_max);

    std::cout << "[before refine] number of cells in mandelbrotMap " << mandelbrotMap.size() << " "
              << "\n";

    // refine nbIter times
    Refine<device_t>::apply(mandelbrotMap, level_min, level_max);
    Kokkos::fence();
    ...
    using Morton2AMR = typename Kokkos::UnorderedMap<uint64_t, amr_key_t, device_t>;
    ...
    Kokkos::parallel_for(
      ...
      Kokkos::RangePolicy<exec_space>(0, sorted_keyVec.size()),
      KOKKOS_LAMBDA(const int & i) {
    ...
    VtkWriter<device_t>::dump("mandelbrot_set_2d.vtk", mandelbrotMap);
...
main(int argc, char * argv[])
{
...

  {
    ...
    kalypsso::MandelbrotCompute<kalypsso::DefaultDevice>::run(
      level_min, level_max, max_capacity_prefactor);

    // kalypsso::MandelbrotCompute<Kokkos::CudaSpace::device_type>::run(
    //   level_min, level_max, max_capacity_prefactor);
    // kalypsso::MandelbrotCompute<Kokkos::OpenMP::device_type>::run(
    //   level_min, level_max, max_capacity_prefactor);
