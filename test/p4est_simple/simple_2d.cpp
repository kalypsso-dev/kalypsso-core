...
/*
 * Additional geometry defined in CanoP:
...
static int
refine_normal_fn(p4est_t * p4est, p4est_topidx_t which_tree, p4est_quadrant_t * quadrant)
{
...
static int
refine_evil3_fn(p4est_t * p4est, p4est_topidx_t which_tree, p4est_quadrant_t * quadrant)
{
  p4est_qcoord_t   u2;
  ...
  if ((which_tree == 0 || which_tree == 1) &&
      (p4est_quadrant_is_equal(&ref, quadrant) || p4est_quadrant_is_ancestor(&ref, quadrant)))
  {
    return 1;
  }
...
static int
coarsen_evil_fn(p4est_t * p4est, p4est_topidx_t which_tree, p4est_quadrant_t * q[])
{
  if (p4est->mpirank >= 2)
  {
    return 1;
...
static int
refine_radius_fn(p4est_t * p4est, p4est_topidx_t which_tree, p4est_quadrant_t * quadrant)
{
 ...
 */
void
my_p4est_vtk_write_file(p4est_t *          p4est,
                        ...
                        double             scale)
{
  ...
  p4est_vtk_context_set_scale(cont, scale);
  ...
  cont = p4est_vtk_write_cell_dataf(cont,
...
main(int argc, char ** argv)
{
   ...
   */
  p4est = p4est_new_ext(mpi->mpicomm, connectivity, 15, 0, 0, sizeof(user_data_t), init_fn, geom);

  double scale = 1.0;

  my_p4est_vtk_write_file(p4est, geom, "simple_2d_new", scale);
  ...
  my_p4est_vtk_write_file(p4est, geom, "simple_2d_refined", scale);

  /* balance */
  p4est_balance(p4est, P4EST_CONNECT_FULL, init_fn);
  my_p4est_vtk_write_file(p4est, geom, "simple_2d_balanced", scale);
  crc = p4est_checksum(p4est);

  /* partition */
  p4est_partition(p4est, 0, NULL);
  my_p4est_vtk_write_file(p4est, geom, "simple_2d_partition", scale);
